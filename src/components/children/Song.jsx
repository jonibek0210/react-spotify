   ...
   return (
      <tr className="max-sm:gap-3 flex gap-5 text-white font-bold items-center">
         ...
         <td className="flex-auto flex flex-col">
            <span className='max-sm:text-[14px] truncate'>{name}</span>
            <span className='text-[#B3B3B3] text-[14px] max-sm:text-[12px] truncate'>{artists[0].name || artists[1].name || ""}</span>
         </td>
         <td className="flex-auto max-sm:hidden">{album.name}</td>
